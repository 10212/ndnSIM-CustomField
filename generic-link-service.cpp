...
GenericLinkService::encodeLpFields(const ndn::PacketBase& netPkt, lp::Packet& lpPacket)
{
  ...
  shared_ptr<lp::ConsumerPositionTag> consumerPositionTag = netPkt.getTag<lp::ConsumerPositionTag>();
  if (consumerPositionTag != nullptr) {
    lpPacket.add<lp::ConsumerPositionField>(*consumerPositionTag);
  }
  //else {
    //lpPacket.add<lp::ConsumerPositionTagField>(0);
  //}
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  //------------------------------------------------------------------------------------------------------------------------------------------
  //added by lahmer ilyas
  //-----------------------------------------------------------------------------------------------------------------------------------------
  
  shared_ptr<lp::AdditionalTag> additionalTag = netPkt.getTag<lp::AdditionalTag>();
  if(additionalTag != nullptr){
    lpPacket.add<lp::AdditionalFieldField>(*additionalTag);
  }
}
...
GenericLinkService::decodeInterest(const Block& netPkt, const lp::Packet& firstPkt)
{
  ...
  // Extract Consumer position
  if (firstPkt.has<lp::ConsumerPositionField>()) {
    interest->setTag(make_shared<lp::ConsumerPositionTag>(firstPkt.get<lp::ConsumerPositionField>()));
  }
  ...
  //addedBy Lahmer
  //----------------------------------------------------------------------------
  if(firstPkt.has<lp::AdditionalFieldField>()){
    interest->setTag(make_shared<lp::AdditionalTag>(firstPkt.get<lp::AdditionalFieldField>()));
  }
...
GenericLinkService::decodeData(const Block& netPkt, const lp::Packet& firstPkt)
{
  ...
  if (firstPkt.has<lp::ConsumerPositionField>()) {
    data->setTag(make_shared<lp::ConsumerPositionTag>(firstPkt.get<lp::ConsumerPositionField>()));
  }
  ...
  //----------------------------------------------------------------------------
  if(firstPkt.has<lp::AdditionalFieldField>()){
    data->setTag(make_shared<lp::AdditionalTag>(firstPkt.get<lp::AdditionalFieldField>()));
  }
